...
if __name__ == '__main__':
	...
	#adapter_r1_info  = FL.access_data().get_adapter_r1_info()
	...
	for i in range(len(input_file_readlines)):
		...
		if i == 0:
			...
			PE_ap_out = PE_a_in.replace('.fastq','') + '.P1.fastq'
			PE_au_out = PE_a_in.replace('.fastq','') + '.U1.fastq'
			print (PE_ap_out)
			print (PE_au_out)

			#b = reverse strand, p = paired, u = unpaired
			PE_bp_out = PE_b_in.replace('.fastq','') + '.P2.fastq'
			PE_bu_out = PE_b_in.replace('.fastq','') + '.U2.fastq'
			PE_bp_out = PE_bp_out.replace(' ','') 
			PE_bu_out = PE_bu_out.replace(' ','') 
